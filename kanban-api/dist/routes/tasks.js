...
import { CreateTaskSchema, UpdateTaskSchema, ClaimTaskSchema, CompleteTaskSchema, BlockTaskSchema, HandoffTaskSchema, TaskQuerySchema } from '../models/task.js';
import { createTask, getTaskById, listTasks, updateTask, deleteTask, claimTask, completeTask, blockTask, handoffTask } from '../services/tasks.js';
...
taskRoutes.post('/:id/block', async (req, res) => {
    const agentId = req.headers['x-agent-id'];
    if (!agentId) {
        return res.status(401).json({ error: 'x-agent-id header required' });
    }
...
taskRoutes.post('/:id/handoff', async (req, res) => {
    const agentId = req.headers['x-agent-id'];
    if (!agentId) {
        return res.status(401).json({ error: 'x-agent-id header required' });
    }
...
taskRoutes.post('/archive-column', async (req, res) => {
