...
// Update a task
export const updateTask = async (id, updates, agentId) => {
    ...
    if (updates.skills_required !== undefined)
        updateData.skills_required = JSON.stringify(updates.skills_required);
    if (updates.timeout_minutes !== undefined)
        updateData.timeout_minutes = updates.timeout_minutes;
    if (updates.blocked_reason !== undefined)
        updateData.blocked_reason = updates.blocked_reason;
...
// Delete a task
export const deleteTask = async (id, agentId) => {
    const existing = await getTaskById(id);
    if (!existing) {
        return { success: false, error: 'Task not found' };
    }
...
export const listTasks = async (query) => {
    ...
    return rows.map(deserializeFromDb).filter((t) => t !== null);
};
// Claim a task (atomic operation)
export const claimTask = async (taskId, agentId) => {
    const now = new Date().toISOString();
...
// Complete a task
export const completeTask = async (taskId, agentId, output, usage) => {
    const now = new Date().toISOString();
    // Build update data
    const updateData = {
        status: 'done',
...
// Block a task
export const blockTask = async (taskId, agentId, reason) => {
    const now = new Date().toISOString();
    const updated = await knex('tasks')
        .where({ id: taskId, status: 'in_progress', claimed_by: agentId })
        .update({
...
export const handoffTask = async (taskId, agentId, output, nextTaskInput) => {
...
export const releaseTimedOutTasks = async () => {
    const now = new Date();
...
export const archiveColumn = async (status) => {
    const now = new Date().toISOString();
